...
//   node server/lockstep/relay.mjs 0    # ephemeral port (printed) — the test suite uses this
//   PORT=8080 node server/lockstep/relay.mjs
//
// ⚠️ NOT `server/match/relay.mjs`, WHICH IS A DIFFERENT SERVER SPEAKING A DIFFERENT
// PROTOCOL. That one is a local stand-in for Azure Web PubSub, carrying the DEDICATED
// MATCH SERVER's snapshots to two thin clients; this one introduces two peers who each
// run the whole match themselves and never looks inside a message. Two online paths,
// two relays, and the names must not be allowed to blur — see LOCK in index.html.
//
