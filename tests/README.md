...
| `trail3s` | The 3-second rule, as a claim about TIME. A player tail covers 2.95s / 660 units at top speed (the old design was 108 units, 0.47s), is bounded to `DOT_MAX` dashes, and a standing player leaves one rather than stacking the window on a spot. Dashes carry a heading recorded with the sample, are **shorter than the gap between them** at speed (a longer one merges into a solid line, which the first build drew) and **longer** at a crawl so they clump. The ball keeps a 3s history, a pitch-relative streak and a `BALL_HOLD` linger so a saved shot still reads — and the suite asserts that a literal 3s ball streak would be **7.6 pitch lengths**, so nobody "fixes" the cap later. Two draws must not age a dash |
