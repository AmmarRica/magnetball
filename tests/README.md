...
| `lobby` | Warm-up lobby: pads in play drop into `warmup` first (keyboard-only doesn't), waiting bots stand off the touchline and neither move nor run the AI, the ball can't be disturbed, sticks and KICK are testable; standing on a half picks your team, the host's Start kicks off and a guest's only readies, sides always come out equal (2v1 → 2v2, everyone-on-one-side splits, solo auto-assigns) and the mode sets the floor; cocktail calibration takes one arrow at a time, ignores short holds and face buttons, accepts stick or dpad, persists the seat's side and is proved by the player then moving *up* the screen |
