...
| `dropin` | A controller joining a match already under way takes over a bot. ⚠️ Seats were handed out exactly ONCE, in `startMatch`, so a pad woken up after the whistle did nothing for the rest of the match — while the warm-up lobby's own help text had been promising "a controller can still join at any point by pressing a button" the whole time. Four lines: (1) it joins on a BUTTON PRESS, never on connection, because a pad waking up in a bag or a browser re-enumerating one must not walk a stranger onto the pitch mid-play; (2) it lands in the seat it would have had at the whistle, compared against `padSeatOrder` ITSELF rather than a hard-coded index — the point is that drop-in and kickoff ask the same question, and an index would still pass if the two drifted together; (3) unplugging hands the body back to the AI keeping its name and stats, since a body nobody drives stands still mid-pitch and a rename makes the award ribbon credit a name nobody saw playing; (4) it cannot perturb the sim — same seed, 600 steps, whole world hashed with the poll firing and without. ⚠️ Two traps recorded in it: the state gates are polled WITHOUT stepping (`w.state='warmup'` on a match not built as a lobby has no `w.lobby` for `stepWarmup`), and the input-cleared-on-release check must be read before the next step or the AI has legitimately steered the body itself and the check passes for free. Plus `localStorage.clear()` must stay OUT of `addInitScript`, which runs again on the reload that the persistence check depends on |
